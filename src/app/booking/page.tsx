...
const CLINIC_INFO = {
  ...
  email: "info@homoeopathy4u.com"
};

// Available baseline slot durations
const BASE_SLOTS = [
...
export default function BookingPage() {
  ...
  useEffect(() => {
    async function generateCalendar() {
      ...
      // Fetch active holiday exceptions from Supabase for this range
      let supabaseHolidays: string[] = [];
      if (supabase) {
        try {
          ...
          if (data) {
            supabaseHolidays = data.map((item: any) => item.exception_date);
          }
        } catch (err) {
          console.warn("Could not fetch holiday exceptions, relying on baseline days:", err);
        }
      }

      let added = 0;
      // Loop up to 45 times to find 30 valid clinic operating days
      for (let i = 0; i < 45 && added < FUTURE_DAYS_LIMIT; i++) {
        ...
        if (checkDate.getDay() !== 0) {
          const isHoliday = supabaseHolidays.includes(dateStr);
          dates.push({
            dateStr,
  ...
  useEffect(() => {
    ...
    async function fetchBookings() {
      ...
      // A. Standard blocked slot: Lunch/break hour (12:30 PM) is disabled by default
      const disabled = [BASE_SLOTS[7]]; 
              ...
              {STEPS.map((step) => {
                ...
                return (
                  <div key={step.id} className="flex flex-col items-center gap-2">
                    <div 
                      className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 font-semibold text-xs md:text-sm ${
                        isActive 
              ...
              {currentStep === 2 && (
                ...
                >
                  <div className="border-b border-slate-100 pb-4">
                    ...
                    <p className="text-xs text-text-body/60 mt-0.5">Select your preferred date and an available 30-minute consultation slot.</p>
                  </div>
                  ...
                  <div className="inline-flex items-center gap-2 bg-[#D6E8EE]/40 border border-[#018ABE]/20 text-[#02457A] px-4 py-2.5 rounded-2xl text-xs font-bold w-full md:w-auto">
                    ...
                    <span>⏱ Appointment Duration: 30 Minutes Per Slot</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                          ...
                          return (
                            <button
                              ...
                              className={`p-3 rounded-2xl border text-center transition-all ${
                                isSelected 
                                  ? "bg-[#02457A] border-[#02457A] text-white shadow-md shadow-brand-primary/10 animate-scale-in" 
                                  : "bg-slate-50/50 border-slate-200 hover:border-brand-secondary text-brand-dark"
                              }`}
                            >
                    ...
                    {/* RIGHT PANEL: AVAILABLE SLOTS WITH SKELETON FEEDBACK */}
                      ...
                      {loadingSlots ? (
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                          {BASE_SLOTS.map((slot) => {
                            ...
                            let btnClass = "bg-emerald-50/70 border-emerald-200/50 text-emerald-700 hover:bg-emerald-100/80";
