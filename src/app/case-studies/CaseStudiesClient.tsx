...
// Fallback case studies list
const fallbackCaseStudies: any[] = [];
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-8">
            ...
            {loading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                {[...Array(2)].map((_, i) => (
                  <div key={i} className="aspect-[16/10] w-full bg-slate-200 animate-pulse rounded-[2rem]" />
                ))}
              </div>
            ) : items.length === 0 ? (
              /* PREMIUM EMPTY STATE */
                      ...
                      <div className="p-8 flex-grow flex flex-col justify-between space-y-4">
                        <div className="space-y-2">
                          {item.patient_condition && (
                            <span className="inline-block px-2.5 py-1 rounded-md bg-brand-light text-brand-dark text-xs font-bold uppercase tracking-wider">
                              {item.patient_condition}
                            </span>
                          )}
                        ...
                        <div className="pt-4 border-t border-slate-50 flex items-center justify-between">
                          <div className="flex items-center gap-1.5 text-xs text-text-body/70 font-semibold">
                            <Calendar size={13} className="text-[#018ABE]" /> {item.created_at ? new Date(item.created_at).toLocaleDateString("en-US", { month: "short", year: "numeric" }) : "June 2026"}
                          </div>
